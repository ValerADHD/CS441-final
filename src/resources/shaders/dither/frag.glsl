...
void main() {
  ...
  float lighting = kamb + kdif * diffuse + kspec * specular;
  lighting = step(texture2D(ditherTex, screen_uv), lighting);

  frag_color = vec4(vertex_color * (kamb + kdif * diffuse + kspec * specular), 1.0);
}
